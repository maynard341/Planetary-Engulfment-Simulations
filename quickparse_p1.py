...
writefile = open("status_p1.txt", "w")
...
#x_data = np.loadtxt(string_default, dtype = float)
        ...
        timelen = len(x_zerocol) #What is the number of the time of cutoff?

        lastecc = x_twocol[timelen - 1] #What is the ending eccentricity?
        ...
        if (e_check < 1.5 and p_check > Rstar):
          print(f"{str(num)}\tSemistable, with eccentricity {lastecc}\n")
          print(f" {str(num)}\tSemistable, with eccentricity {lastecc}\n", file=writefile)
        elif (p_check < Rstar):
